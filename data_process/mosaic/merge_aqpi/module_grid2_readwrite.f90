...
! Open GRIB2 file 
     ...
     if (iret.eq.0) then
        call baopenw(wrtfile,trim(outfilename),iret)
        if (iret.ne.0) then
           write(*,*) 'Error open ',trim(outfilename)
           stop 123
        endif
