...
subroutine get_var_nc_short_3d(this,varname,nd1,nd2,nd3,field)
!
! read in one field 
!
  use netcdf
...
!
  ...
  character(len=*),intent(in) :: varname  ! name of the field to read
  integer, intent(in) :: nd1,nd2,nd3      !  size of array dval
  integer(2), intent(out) :: field(nd1,nd2,nd3) !  values of the field read in
  integer :: ilength
!
  integer(2),allocatable :: temp(:)
!
  character*40,parameter :: thissubname='get_var_nc_short_3d'
!
  integer :: i,j,k
  integer :: length2d
  integer :: istart,iend
...
!
  ...
  deallocate(temp)
!
end subroutine get_var_nc_short_3d
...
subroutine create_nc(this,filename,varname,nd1,nd2,field)
!
! create a netcdf file and write out a 2d field 
!
! prgmmr:  Eric James          org: GSD           date: 2018-04-20
!
! abstract:
!
  use netcdf
...
!
  ...
  real(4), intent(in) :: field(nd1,nd2) !  values of the field read in
!
  integer :: fileID,id1,id2,fieldID,status
  integer :: i,j
  integer :: istart,iend
  integer, dimension(2) :: dimIDs
!
  status = nf90_create(filename, nf90_share, fileID)
!
  status = nf90_def_dim(fileID, 'x', nd1, id1)
  status = nf90_def_dim(fileID, 'y', nd2, id2)
  dimIDs = (/ id1, id2 /)

  status = nf90_def_var(fileID, varname, nf90_double, dimIDs, fieldID)
  status = nf90_enddef(fileID)

  status = nf90_put_var(fileID, fieldID, field)
