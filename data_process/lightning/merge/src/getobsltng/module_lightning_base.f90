...
module module_lightning_base
   ...
   public :: lightninglocation

   private

!  Define a lightning observation type.

   type lightninglocation
      real(r_single) :: lon  ! stroke longitude
      real(r_single) :: lat  ! stroke latitiude
   end type lightninglocation

!  Define a lightning observation type to contain actual data.

   type, extends(lightninglocation) :: lightningbase
!  HOW DOES THIS POINTER THING WORK?
      type(lightningbase), pointer :: next => NULL()
...
!                                              GLM has flash_quality_flag
   ...
   end type lightningbase

   contains

      subroutine list_obsbase(this)

!     This subroutine lists the contents of a base lightning observation

         class(lightningbase) :: this
...
!     This subroutine allocates memory for base lightning observation variables
!     Input variables:
...
!     This subroutine releases memory associated with lightning observations
...
end module module_lightning_base
